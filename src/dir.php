...
if (strpos(stripslashes($dir), "../") === false) {} else {
    echo "Can't go to parent directories!";
    exit();
}
...
if (! file_exists($settings->get('path') . $dir)) {
    ...
    if (isset($dir)) {
        ...
        ?>
                ...
                <table class="table table-striped">
                    <tr>
                        <td><a href="<?php echo $parentURL; ?>"
                            title="<?php _BACKTOPARRENT; ?>"> <i
                                class="fa fa-folder-open"
                                aria-hidden="true"
                                style="color: orange; margin-right: 4px;"></i>[<?php echo _BACKTOPARRENT; ?>]
                				</a></td>
                        <td class="hidden-sm-down"></td>
                        <td style="text-align: right">Multi-Delete-&gt;</td>
                        <td style="text-align: right"><a class="delete"
                            href="javascript:document.multidir.submit()"
                            data-file="Multiple Files"><i
                                class="fa fa-trash-o" style="color: red"
                                aria-hidden="true"
                                title="<?php echo _DELETE; ?>"></i></a>
    ...
    foreach ($entrys as $entry) {
        ?>
		                <tr>
                        <td><a
                            href="dir.php?dir=<?php echo urlencode($dir . $entry); ?>"
                            title="<?php echo $entry; ?>"> <i
                                class="fa fa-folder" aria-hidden="true"
                                style="color: orange; margin-right: 4px;"></i><?php echo $entry; ?>
		                		</a></td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td style="text-align: right">
    			            ...
    			            <?php if ($settings->get('enable_maketorrent')) { ?>
			                    <a class="makeTorrent" href="#"
                            data-url="maketorrent.php?path=<?php echo urlencode($dir . $entry); ?>"><i
                                class="fa fa-external-link"
                                style="color: #5CB85C"
                                aria-hidden="true" title="Make Torrent"></i></a> 
    			            <?php } ?>
    			    
			                <?php if ($settings->get('enable_file_download')) { ?>
	                    		<a
                            href="dir.php?tar=<?php echo urlencode($dir . $entry); ?>"><i
                            ...
                            name="file[]"
                            value="<?php echo urlencode($dir . $entry); ?>">
							<?php } ?>
    			            </td>
                    </tr>
    			  <?php
    ...
    foreach ($entrys as $entry) {
        ...
        ?>
                        ...
                        <td>
							...
							<?php if ($settings->get('enable_file_download')) { ?>
								<a href="dir.php?down=<?php echo urlencode($dir.$entry); ?>"> <i
                                class="fa fa-file" aria-hidden="true"
                                style="color: orange; margin-right: 4px;"></i><?php echo $entry; ?>
    			                </a>
			                <?php } else { ?>
    			                    <i class="fa fa-file" aria-hidden="true"
                            style="color: orange; margin-right: 4px;"></i>
    			                    <?php echo $entry; ?>
							<?php } ?>
                        ...
                        <td style="text-align: right">
			                ...
			                <?php if( $settings->get('enable_view_nfo') && (( substr( strtolower($entry), -4 ) == ".nfo" ) || ( substr( strtolower($entry), -4 ) == ".txt" ))  ) { ?>
    			            	<a
                            href="viewnfo.php?path=<?php echo urlencode(addslashes($dir.$entry)); ?>"
                            title="View <?php echo $entry; ?>"><i
			                ...
			                <?php if ($settings->get('enable_file_download')) { ?>
    			            	<a
                            href="dir.php?down=<?php echo urlencode($dir.$entry); ?>"><i
                                class="fa fa-download"
                                style="color: #5CB85C"
