...
$sql = "SELECT user_id, file, action, ip, ip_resolved, user_agent, time FROM tf_log WHERE " .
         $sqlForSearch . "action!=" . $db->qstr($cfg["constants"]["hit"]) .
         " ORDER BY time desc";
...
while (list ($user_id, $file, $action, $ip, $ip_resolved, $user_agent, $time) = $result->FetchRow()) {
    ...
    $ip_info = htmlentities($ip_resolved, ENT_QUOTES) . "<br>" .
             htmlentities($user_agent, ENT_QUOTES);
    
    $output .= "<tr>";
    if (IsUser($user_id)) {
        $output .= "<td><a href=\"" . $settings->get('base_url') .
                 "/message.php?to_user=" . $user_id . "\"><img src=\"" .
                 $user_icon . "\" alt=\"\" title=\"" . _SENDMESSAGETO . " " .
                 $user_id . "\" />" . $user_id . "</a>&nbsp;&nbsp;</td>";
    } else {
        $output .= "<td><img src=\"" . $user_icon .
                 "\" alt=\"\" title=\"n/a\" />" . $user_id . "&nbsp;&nbsp;</td>";
    }
    $output .= "<td><div class=\"tiny\">" . htmlentities($action, ENT_QUOTES) .
             "</div></td>";
    $output .= "<td><div class=\"tiny\" style=\"text-align:left\">";
    $output .= htmlentities($file, ENT_QUOTES);
    $output .= "</div></td>";
    $output .= "<td><div class=\"tiny\" style=\"text-align:left\"><a href=\"javascript:void(0)\" onclick=\"return overlib('" .
             $ip_info .
             "<br>', STICKY, CSSCLASS);\" onmouseover=\"return overlib('" .
             $ip_info .
             "<br>', CSSCLASS);\" onmouseout=\"return nd();\" class=tiny><img src=\"" .
             $settings->get('base_url') .
             "/images/properties.png\" alt=\"\" />" .
             htmlentities($ip, ENT_QUOTES) . "</a></div></td>";
    $output .= "<td><div class=\"tiny\" style=\"text-align:center\">" .
             date(_DATETIMEFORMAT, $time) . "</div></td>";
    $output .= "</tr>";
...
if ($inx == 0) {
    $output = "<tr><td colspan=6><center><strong>-- " . _NORECORDSFOUND .
             " --</strong></center></td></tr>";
}

$prev = ($min - $offset);
if ($prev >= 0) {
    $prevlink = "<a href=\"activity.php?min=" . $prev . "&user_id=" . $user .
             "&srchFile=" . $srchFile . "&srchAction=" . $srchAction . "\">";
    $prevlink .= "&lt;&lt;" . $min . " " .
             _SHOWPREVIOUS . "]</a> &nbsp;";
}
if ($inx >= $offset) {
    $morelink = "<a href=\"activity.php?min=" . $max .
             "&user_id=" . $user . "&srchFile=" . $srchFile . "&srchAction=" .
             $srchAction . "\">";
...
<div class="container">
	<div class="row">
				...
				<table class="table table-striped">
					<tr>
						<td><strong><?php echo _ACTIVITYSEARCH ?></strong>&nbsp;&nbsp;&nbsp;
							...
							value="<?php echo $srchFile ?>" style="width: 30"> &nbsp;&nbsp;
       						<?php echo _ACTION ?>:
        ...
        if (is_array($cfg["constants"])) {
            foreach ($cfg["constants"] as $action) {
                ...
                if ($action != $cfg["constants"]["hit"]) {
                    ...
                    echo "<option value=\"" . htmlentities($action, ENT_QUOTES) .
                             "\" " . $selected . ">" .
                             htmlentities($action, ENT_QUOTES) . "</option>";
                }
            }
        }
        ?>
        ...
        for ($inx = 0; $inx < sizeof($users); $inx ++) {
            ...
            echo "<option value=\"" . htmlentities($users[$inx], ENT_QUOTES) .
                     "\" " . $selected . ">" .
                     htmlentities($users[$inx], ENT_QUOTES) . "</option>";
        }
        ?>
