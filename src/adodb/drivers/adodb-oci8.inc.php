...
class ADODB_oci8 extends ADOConnection
{
 ...
 // MetaType('DATE') returns 'D' (datetime==false) or 'T' (datetime == true)
    ...
    function _connect($argHostname, $argUsername, $argPassword, $argDatabasename, $mode = 0)
    {
        ...
        if ($mode == 1) {
            $this->_connectionID = ($this->charSet) ? OCIPLogon($argUsername, $argPassword, $argDatabasename) : OCIPLogon($argUsername, $argPassword, $argDatabasename, $this->charSet);
            if ($this->_connectionID && $this->autoRollback)
                OCIrollback($this->_connectionID);
        } else 
            if ($mode == 2) {
                $this->_connectionID = ($this->charSet) ? OCINLogon($argUsername, $argPassword, $argDatabasename) : OCINLogon($argUsername, $argPassword, $argDatabasename, $this->charSet);
            } else {
                $this->_connectionID = ($this->charSet) ? OCILogon($argUsername, $argPassword, $argDatabasename) : OCILogon($argUsername, $argPassword, $argDatabasename, $this->charSet);
            }
